const COLOR_CT = "rgba(56, 123, 238, 1.0)";
const COLOR_T = "rgba(242, 162, 37, 1.0)";
...
const PLAYER_GREEN = "rgba(11, 216, 115, 1.0)";
...
function updateTopPanel() {
  ...
  $("#left_team #main")
    ...
    .css("color", teams.left.side == "ct");
  $("#right_team #main")
    .text(teams.right.name)
    .css("color", teams.right.side == "ct");
  //#endregion
...
function updateRoundNow(round, map) {
  ...
  $("#round_number").text("Round " + round_now);
  ...
  if (phase) {
    ...
    if (round.win_team == "CT") {
      ...
      if (teams.left.side == "t") {
        ...
        // * T alert on Right
        $("#right_team_a").attr("src", "/storage/" + teams.right.logo);
...
function fillObserved(obs) {
 ...
 // Limit the alias name to a maximum of 14 characters
const truncatedAlias = obs.name.length > 14 ? obs.name.slice(0, 14) : obs.name;
...
$("#obs_realname_text").text(
  obs.real_name && obs.real_name !== obs.name ? obs.real_name : ""
);
  ...
  if (disp_avatars) {
    if (disp_player_avatars) {
      if (obs.hasOwnProperty("avatar")) {
        ...
        // Just Use Team Logo
        $("#obs_img").attr("src", "/storage/player_silhouette.webp");
      }
    } else {
  ...
  $("#obs_kills_text").text(stats.kills).css("color", team_color);
  $("#obs_assists_a").css("color", team_color);
  $("#obs_assists_text").text(stats.assists);
  $("#obs_deaths_d").css("color", team_color);
  $("#obs_deaths_text").text(stats.deaths).css("color", team_color);
...
if (slot === 10) {
  $player.find(".player_slot_number").text(0); // Special handling for slot 9
} else {
...
$player.find(".player_slot_number").css("display", dead ? "none" : "block");
  ...
  if (dead) {
    ...
    if (side.substr(8) == "left") {
      $player.find("#player_stat").css("left", "115px").css("top", "26px");
      ...
      $player.find("#player_stat").css("right", "115px").css("top", "26px");;
      $player.find(".player_section_top").css("width", "200px");
      $player.find(".player_section_bottom").css("width", "200px");
    ...
    if (side.substr(8) == "left") {
      $player.find("#player_stat").css("left", "215px").css("top", "0px");
      ...
      $player.find("#player_stat").css("right", "215px").css("top", "0px");;
      $player.find(".player_section_top").css("width", "300px");
      $player.find(".player_section_bottom").css("width", "300px");
      ...
      $player.find("#player_alias_text").css("color", "rgba(255,255,255,1");
      $player.find("#player_current_money_text").css("color", "#0bd873");
      $player.find("#player_skull").css("right", "10px");
      $player.find("#player_round_kills_text").css("right", "0px");
    }
  }
  ...
  if (slot == obs_slot) {
    const gradient = team === "ct" 
      ? "linear-gradient(to left, rgba(56, 123, 238, 0), rgba(56, 123, 238, 1))" 
      : "linear-gradient(to right, rgba(242, 162, 37, 0), rgba(242, 162, 37, 1))";
    $player
  ...
  $bottom.find("#player_current_money_text").css("#0bd873", dead ? COLOR_WHITE_HALF : "#a7d32e");
